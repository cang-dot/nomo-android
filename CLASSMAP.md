...
| Android 分享与持久导入 | `src-tauri/src/android_intent.rs`, `src-tauri/src/mobile_imports.rs` | `src-tauri/src/mobile.rs`, `src-tauri/gen/android/app/src/main/java/com/nomo/desktop/MainActivity.kt`, `SharedText.kt`, `src-tauri/src/config/mod.rs` | 分享文本/链接归一化、内部文件原子发布、旧缓存与配置引用迁移；不回写来源、不按年龄删除 |
